        ...
        try {
            ...
            for each (ManagerTable ^ ban in danhSachBan) {
                ...
                for each (MonAn ^ monAn in ban->DanhSachMon) {
                    ...
                    monAnStr += String::Format("{0}:{1}:{2}:{3}:{4}:{5};",
                        monAn->ID, monAn->LoaiMon, monAn->TenMon, monAn->Gia, monAn->Anh, monAn->SoLuong);
                }
                if (monAnStr != "") monAnStr = monAnStr->Substring(0, monAnStr->Length - 1); // Xóa dấu ; cuối
                String^ line = String::Format("{0}|{1}|{2}|{3}", ban->ID, ban->SoBan, ban->TrangThai, monAnStr);
