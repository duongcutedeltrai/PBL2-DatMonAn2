...
namespace PBL2DatMonAn {
    ...
    void User::GhiDanhSachTaiKhoan(List<User^>^ dsTaiKhoan, String^ UserfilePath) {
        try {
            System::IO::StreamWriter^ writer = gcnew System::IO::StreamWriter(UserfilePath, false, System::Text::Encoding::UTF8);
    ...
    List<User^>^ User::DocDanhSachTaiKhoan(String^ UserfilePath) {
        List<User^>^ dsTaiKhoan = gcnew List<User^>();
        try {
            System::IO::StreamReader^ reader = gcnew System::IO::StreamReader(UserfilePath, System::Text::Encoding::UTF8);
            String^ dong;
